...
* in the context of Brexit Referendum campaign. The model simulates a prototype scenario in which Remain and Leave
* position are more or less equally supported (Remain starts with s ahigher value than Leave) before the start of the campaign (media effect)
...
global {
	...
	float perc_leave_voters <- 0.4709-0.05;
	float perc_remain_voters <- 0.5291-0.05;
	...
	reflex media_effect {
		...
		// pro-Leave
			...
			// pro-Leave media has an effect, sums 0.1 to leave
				...
				// pro-Remain media does not have an effect becasue the neighborhood is homogenous
...
experiment tie_campaign type: gui  {
...
experiment tie type: batch repeat:1 until: (cycle = 30) {
	
	reflex save_results when: (cycle > 1){
    ask simulations {
    save [self.name, [n_leave_voters, n_remain_voters, n_undecided_voters]] to: "tie_batch_small_wordl.csv" type: "csv" rewrite:false;
    }
  }
	// To choose the layout in the experiment 
	user_command "Layout graph" {
