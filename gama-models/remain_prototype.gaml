...
* of the voter support the Remain position before the start of the campaign (media effect)
...
global {
	...
	float perc_leave_voters <- 0.3192-0.05;
	float perc_remain_voters <- 0.6808-0.05;
...
experiment remain_campaign type: gui  {
	
	// For a side-by-side model comparison
	/*float seedValue <- 120.0;
    float seed <- seedValue; // force the value of the seed with the values from Leave model
 
    init {
        create simulation with:[seed::seedValue,perc_leave_voters::0.6103-0.05,perc_remain_voters::0.3897-0.05,perc_undecided_voters::0.1];
...
experiment remain type: batch repeat:1 until: (cycle = 30) {
	
	reflex save_results when: (cycle > 1){
    ask simulations {
    save [self.name, [n_leave_voters, n_remain_voters, n_undecided_voters]] to: "remain_batch_small_wordl.csv" type: "csv" rewrite:false;
